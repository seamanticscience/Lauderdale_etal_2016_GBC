...
%%    
    ...
    elseif nc_isvar([modeldir,'/gmDiag.',num2str(start_iter,'%010d'),'.glob.nc'],'GM_U_RES') && ...
        ...
        %[uflux,vflux,wflux]=mit_advect_flux(timestep,advscheme,dx,dy,dz,rax,ray,rac,...
        %                                  mask,uvel,vvel,wvel,scalar);
        [utflux,vtflux,wtflux]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
        ...
        [usflux,vsflux,wsflux]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(ures,'==',NaN,0),change(vres,'==',NaN,0),change(wres,'==',NaN,0),change(salt,'==',NaN,0));
        [ucflux,vcflux,wcflux]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(ures,'==',NaN,0),change(vres,'==',NaN,0),change(wres,'==',NaN,0),change(dic,'==',NaN,0));
        [uaflux,vaflux,waflux]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(ures,'==',NaN,0),change(vres,'==',NaN,0),change(wres,'==',NaN,0),change(alk,'==',NaN,0));
        [upflux,vpflux,wpflux]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
        ...
        %[uflux,vflux,wflux]=mit_advect_flux(timestep,advscheme,dx,dy,dz,rax,ray,rac,...
        %                                  mask,uvel,vvel,wvel,scalar);
        [utflux_eulmean,vtflux_eulmean,wtflux_eulmean]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(uvel,'==',NaN,0),change(vvel,'==',NaN,0),change(wvel,'==',NaN,0),change(theta,'==',NaN,0));
        [usflux_eulmean,vsflux_eulmean,wsflux_eulmean]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(uvel,'==',NaN,0),change(vvel,'==',NaN,0),change(wvel,'==',NaN,0),change(salt,'==',NaN,0));
        [ucflux_eulmean,vcflux_eulmean,wcflux_eulmean]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(uvel,'==',NaN,0),change(vvel,'==',NaN,0),change(wvel,'==',NaN,0),change(dic,'==',NaN,0));
        [uaflux_eulmean,vaflux_eulmean,waflux_eulmean]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
            change(uvel,'==',NaN,0),change(vvel,'==',NaN,0),change(wvel,'==',NaN,0),change(alk,'==',NaN,0));
        [upflux_eulmean,vpflux_eulmean,wpflux_eulmean]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
            grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
        ...
        if strcmp(eddy_method,'residual')
           ...
           if isfield(gmdiag,'GM_U_EDD') && isfield(gmdiag,'GM_V_EDD') && isfield(gmdiag,'GM_W_EDD')
            ...
            [utflux_eddies,vtflux_eddies,wtflux_eddies]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
                grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
                change(uvel_eddy,'==',NaN,0),change(vvel_eddy,'==',NaN,0),change(wvel_eddy,'==',NaN,0),change(theta,'==',NaN,0));
            [usflux_eddies,vsflux_eddies,wsflux_eddies]=mit_advect_flux(timestep,advscheme,grid.dxg,grid.dyg,grid.dz,...
                grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
                change(uvel_eddy,'==',NaN,0),change(vvel_eddy,'==',NaN,0),change(wvel_eddy,'==',NaN,0),change(salt,'==',NaN,0));
            [ucflux_eddies,vcflux_eddies,wcflux_eddies]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
                grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
                change(uvel_eddy,'==',NaN,0),change(vvel_eddy,'==',NaN,0),change(wvel_eddy,'==',NaN,0),change(dic,'==',NaN,0));
            [uaflux_eddies,vaflux_eddies,waflux_eddies]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
                grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
                change(uvel_eddy,'==',NaN,0),change(vvel_eddy,'==',NaN,0),change(wvel_eddy,'==',NaN,0),change(alk,'==',NaN,0));
            [upflux_eddies,vpflux_eddies,wpflux_eddies]=mit_advect_flux(timestep,ptradvscheme,grid.dxg,grid.dyg,grid.dz,...
                grid.rax,grid.ray,grid.rac,grid.umask,grid.vmask,grid.cmask,...
...
[dthetadt]=mit_integrate_vert(grid.dz,kave,grid.cmask,gtheta);
[dsaltdt ]=mit_integrate_vert(grid.dz,kave,grid.cmask,gsalt );
[ddicdt  ]=mit_integrate_vert(grid.dz,kave,grid.cmask,gdic  );
[dalkdt  ]=mit_integrate_vert(grid.dz,kave,grid.cmask,galk  );
[dpo4dt  ]=mit_integrate_vert(grid.dz,kave,grid.cmask,gpo4  );
   
[po4_bio_flux]=mit_integrate_vert(grid.dz,kave,grid.cmask,-dic_bio_ave+dic_pflux_ave+max(KDOPRemin.*dop,0));
[dic_carb_flux]=mit_integrate_vert(grid.dz,kave,grid.cmask,dic_car_ave);
...
[theta_adv_horz,theta_adv_vert]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
    change(utflux,'==',NaN,0),change(vtflux,'==',NaN,0),change(wtflux,'==',NaN,0));
[salt_adv_horz ,salt_adv_vert ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
    change(usflux,'==',NaN,0),change(vsflux,'==',NaN,0),change(wsflux,'==',NaN,0));
[dic_adv_horz  ,dic_adv_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
    change(ucflux,'==',NaN,0),change(vcflux,'==',NaN,0),change(wcflux,'==',NaN,0));
[alk_adv_horz  ,alk_adv_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
    change(uaflux,'==',NaN,0),change(vaflux,'==',NaN,0),change(waflux,'==',NaN,0));
[po4_adv_horz  ,po4_adv_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
...
switch lower(vertmix)
    case('')
        ...
        [theta_diff_horz,theta_diff_vert]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
            change(utdiff,'==',NaN,0),change(vtdiff,'==',NaN,0),change(wtediff+wtidiff,'==',NaN,0));
        [salt_diff_horz,salt_diff_vert ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
            change(usdiff,'==',NaN,0),change(vsdiff,'==',NaN,0),change(wsediff+wsidiff,'==',NaN,0));
        [dic_diff_horz,dic_diff_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
            change(ucdiff,'==',NaN,0),change(vcdiff,'==',NaN,0),change(wcediff+wcidiff,'==',NaN,0));
        [alk_diff_horz,alk_diff_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
            change(uadiff,'==',NaN,0),change(vadiff,'==',NaN,0),change(waediff+waidiff,'==',NaN,0));
        [po4_diff_horz,po4_diff_vert  ]=mit_calc_divergence(grid.dz,grid.volc,kave,grid.cmask,...
            change(updiff,'==',NaN,0),change(vpdiff,'==',NaN,0),change(wpediff+wpidiff,'==',NaN,0));
    case('diffusion')
        % Get the explicit vertical diffusion
        [utdiff,vtdiff,wtediff]=mit_diffuse_flux(grid.dxc,grid.dyc,grid.dzc,...
            grid.umask,grid.vmask,grid.cmask,grid.rax,grid.ray,grid.rac,...
            gm_kux,gm_kuz,gm_kvy,gm_kvz,gm_kwx,gm_kwy,change(theta,'==',NaN,0));
        [usdiff,vsdiff,wsediff]=mit_diffuse_flux(grid.dxc,grid.dyc,grid.dzc,...
            grid.umask,grid.vmask,grid.cmask,grid.rax,grid.ray,grid.rac,...
            gm_kux,gm_kuz,gm_kvy,gm_kvz,gm_kwx,gm_kwy,change(salt,'==',NaN,0));
        [ucdiff,vcdiff,wcediff]=mit_diffuse_flux(grid.dxc,grid.dyc,grid.dzc,...
            grid.umask,grid.vmask,grid.cmask,grid.rax,grid.ray,grid.rac,...
            gm_kux,gm_kuz,gm_kvy,gm_kvz,gm_kwx,gm_kwy,change(dic,'==',NaN,0));
        [uadiff,vadiff,waediff]=mit_diffuse_flux(grid.dxc,grid.dyc,grid.dzc,...
            grid.umask,grid.vmask,grid.cmask,grid.rax,grid.ray,grid.rac,...
            gm_kux,gm_kuz,gm_kvy,gm_kvz,gm_kwx,gm_kwy,change(alk,'==',NaN,0));
        [updiff,vpdiff,wpediff]=mit_diffuse_flux(grid.dxc,grid.dyc,grid.dzc,...
            grid.umask,grid.vmask,grid.cmask,grid.rax,grid.ray,grid.rac,...
        ...
        wtidiff=mit_difimpl_flux(timestep,iteration,grid.rac,grid.dz,grid.dzc,grid.cmask,...
            diffkz,theta);
        %        change(diffkz,'==',NaN,0),change(theta,'==',NaN,0));
        wsidiff=mit_difimpl_flux(timestep,iteration,grid.rac,grid.dz,grid.dzc,grid.cmask,...
            diffkz,salt);
        %        change(diffkz,'==',NaN,0),change(salt,'==',NaN,0));
        wcidiff=mit_difimpl_flux(timestep,iteration,grid.rac,grid.dz,grid.dzc,grid.cmask,...
            diffkz,dic);
        %        change(diffkz,'==',NaN,0),change(dic,'==',NaN,0));
        waidiff=mit_difimpl_flux(timestep,iteration,grid.rac,grid.dz,grid.dzc,grid.cmask,...
            diffkz,alk);
        %        change(diffkz,'==',NaN,0),change(alk,'==',NaN,0));
        wpidiff=mit_difimpl_flux(timestep,iteration,grid.rac,grid.dz,grid.dzc,grid.cmask,...
            diffkz,po4);
